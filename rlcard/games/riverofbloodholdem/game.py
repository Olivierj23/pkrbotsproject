...
from rlcard.games.riverofbloodholdem import Dealer
from rlcard.games.riverofbloodholdem import Player
from rlcard.games.riverofbloodholdem import Judger
from rlcard.games.riverofbloodholdem import Round, Action
...
class Stage(Enum):
    ...
    TURN = 2
    RIVER = 3
    # MODIFICATION: modified to include RUN's
    RUN = 4
    SHOWDOWN = 5
...
class NolimitholdemGame(Game):
    def __init__(self, allow_step_back=False, num_players=2):
        ...
        # config players
        self.init_chips = [400] * num_players # MODIFICATION: changed 100 to 400
    ...
    def step(self, action):
        ...
        if self.round.is_over():
            ...
            if self.round_counter == 2:
                ...
                if len(self.players) == np.sum(players_in_bypass):
                    self.round_counter += 1
            # MODIFICATION: added addditional round counter conditional for RUN
    ...
    def step_back(self):
        ...
        if len(self.history) > 0:
            ...
            # MODIFICATION: Adjusted to make up for additional runs in river of blood
            if self.round_counter < 4:
                self.stage = Stage(self.round_counter)

            elif self.round_counter >= 4 and str(self.public_cards[-1])[1] in ['D', 'H']:
                self.stage = Stage(4)

            elif self.round_counter >=4 and str(self.public_cards[-1])[1] in ['S', 'C']:
                self.stage = Stage(5)
    ...
    def is_over(self):
        ...
        # MODIFICATION: added round ending condition that allows the game to continue if the last card is red
        if self.round_counter >= 4 and len(self.public_cards) > 0:
            if str(self.public_cards[-2])[1] in ['S', 'C']:
                return True
            else:
                pass
        return False
