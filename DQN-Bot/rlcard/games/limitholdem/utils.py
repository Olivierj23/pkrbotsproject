...
class Hand:
    ...
    def _get_High_cards(self):
        ...
        Returns:
            (list): best five hand cards after sort
        '''
        length_of_all_cards = len(self.all_cards)
        High_cards = self.all_cards[length_of_all_cards-5:length_of_all_cards]
        return High_cards
...
def final_compare(hands, potential_winner_index, all_players):
    ...
    if eval7.evaluate([eval7.Card(s) for s in hands[0]]) > eval7.evaluate([eval7.Card(s) for s in hands[1]]):
        return [1, 0]

    elif eval7.evaluate([eval7.Card(s) for s in hands[1]]) > eval7.evaluate([eval7.Card(s) for s in hands[0]]):
        return [0, 1]
